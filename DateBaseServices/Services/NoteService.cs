...
namespace DateBaseServices.Services
{
    ...
    public class NoteService : DefaultService
    {
        ...
        public List<Note> GetNotesByCatIdForUserByUserId(int categoryId, int userId, string token)
        {
            if (!SecurityService.Service.SecurityService.ValidateCurrentToken(token, userId))
                throw new DbServiceException($"Токен({token}) недействителен.");
            ...
            var notes = _db.NoteCategoryLinkers.Include(l => l.Note).Where(l => l.CategoryId == categoryId && !l.Note.IsDeleted)
                .Select(l => l.Note).ToList();
